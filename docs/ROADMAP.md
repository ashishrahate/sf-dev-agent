...
| Real-org pressure test of the 4-layer orchestrator | Must-have for prod | 2-3 days | User-driven, manual execution. Canonical protocol at [`docs/PRESSURE_TEST.md`](PRESSURE_TEST.md) — re-runnable against any org. First correctness-pass against `AgentforceOrg` (22 ApexClasses) tracked in [`docs/sessions/2026-05-07.md`](sessions/2026-05-07.md). Scale-stress run (1K+ components) still held until larger org is authorized; protocol unchanged across run modes. |
...
| Subagents (Claude-Code-style spawnable child agents) | Nice-to-have | ~7 days across 3 slices | Parked 2026-05-07 after design brainstorm. Motivation: context-window isolation as orgs scale (raw tool results currently glue into the parent transcript). Locked decisions: predefined types only in v1 (`code-explorer` + `schema-explorer`); subagents inherit parent's `LLMProvider` (no multi-model in Slice 1 — defers cost-asymmetry win); bundled definitions at `src/sf_dev_agent/subagents/definitions/*.md` with user override at `~/.sf-agent/agents/*.md` (mirrors knowledge-base loader); hooks into existing interception slot in `agent.py:_execute_tool` next to `submit_plan` / `request_resume`. Slice 1 ~2-3d (explorer MVS, ~25-35 tests). Slice 2 ~1-2d (multi-model + token accounting + cost guardrails). Slice 3 ~4-5d (specialists + parallel spawning + custom user definitions). Hold until pressure-test telemetry confirms context-window pressure is the actual bottleneck. Full design space + alternatives + risks: plan file at `~/.claude/plans/okay-lets-brainstorm-in-fuzzy-minsky.md`. |
...
| Claude-Code-style REPL polish (v1) | ✅ shipped 2026-04-29 | — | New `repl_ui.py` module: consistent header/result/error/blocked rendering for tool calls, spinner via `console.status` while tools run, compact key=value input summary. Wired into `agent.py:_execute_tool` + the streaming `on_text` callback. |
| REPL UI v2 — inline diffs (slice 1) | ✅ shipped 2026-05-08 | — | `render_file_write_diff` in `repl_ui.py` prints a colored unified diff between header and OK line for `file_write` tool calls. New file → all-additions diff; no-op write → silent skip. Path-traversal-safe pre-write capture in `agent.py` mirrors the executor's own validation. Slices 2 (syntax-highlighted code in tool results) + 3 (collapsible blocks via tool_use_id capture) still deferred. |
| Operating modes — slice A (core infra) | ✅ shipped 2026-05-09 | — | New `AgentMode` enum (PLAN / EXECUTION / GENERAL) in `models/schemas.py`. `AgentLoop(mode=...)` defaults to PLAN — backwards-compatible. `run()` dispatches: PLAN → existing planning + approval flow; EXECUTION + GENERAL → new `_run_direct()` that skips Phase 1. Mode-aware gating in `_execute_tool`: PLAN keeps two-stage gate; EXECUTION passes writes through; GENERAL prompts inline (`yes / no / always / cancel`, default `no`, non-TTY auto-deny, Ctrl+C / EOF treated as cancel). Caller-injectable `write_allowlist` set lets the REPL share an "always" allowlist across tasks. `submit_plan` filtered from tool defs in non-PLAN modes; defensive intercept clarifies if called anyway. System prompt has new `{{AGENT_MODE_INSTRUCTIONS}}` placeholder; PLAN block is empty (existing prompt body authoritative); EXECUTION + GENERAL get loud override blocks. KeyboardInterrupt from inline-approval cancel now caught around tool dispatch (uses existing `_handle_interrupt`). 22 new tests in `test_agent_modes.py` covering all gating paths + edge cases. |
| Operating modes — slice B (REPL UX) | ✅ shipped 2026-05-09 | — | `ReplSession.mode` (default PLAN) + per-session `write_allowlist`. Shift+Tab keybinding cycles general → plan → execution → general; the binding mutates session state and `event.app.invalidate()` repaints the bottom toolbar live. Mode rendered in bottom toolbar (plain text, mode-first field) and in the startup banner (color-coded: green plan, yellow general, red execution). New `/mode` slash command (show / set / reject invalid). Pre-dispatch autosuggest in `_dispatch_agent`: when input contains code-change keywords (create / write / update / deploy / fix / etc.) AND mode != PLAN AND stdin is a TTY, soft-prompt to flip to plan mode for the next task. AgentLoop construction in `_dispatch_agent` threads `session.mode` + `session.write_allowlist`. 17 new tests in `test_repl.py` covering cycle order, label colors, status dict, toolbar/banner rendering, /mode show / set / round-trip / reject, autosuggest TTY/keyword/mode gates, and yes/no flip. |
| Operating modes — slice C (persistence + resume) | ✅ shipped 2026-05-09 | — | New `mode TEXT NOT NULL DEFAULT 'plan'` column on `tasks` table. `WorkingMemoryStore` runs an idempotent ALTER TABLE migration on construction so existing pre-slice-C DBs upgrade in place; legacy rows default to `plan`. `TaskRow.mode` field with `plan` fallback for very old rows. `create_task(mode=...)` parameter persists at task creation. `AgentLoop.run()` writes `mode=self.mode.value` to the row + uses initial status PLANNING for plan mode and EXECUTING for non-plan modes. `AgentLoop.resume()` reads mode from the persisted row and overrides any caller-supplied default — a task started in plan mode can't accidentally become execution on resume just because the REPL session changed. `_run_execution_only` banner is mode-aware (Phase 2 wording for plan, "Resuming in <mode>" otherwise). 10 new tests in `test_mode_persistence.py` covering schema column presence, default-to-plan, explicit-mode round-trip, legacy-DB ALTER migration, idempotent migration, all-three-mode persistence through `run()`, resume preferring row mode, and graceful fallback for unknown row values. |
| Auto-reindex after write | ✅ shipped 2026-05-09 | — | After a successful `file_write` or `sf_source_deploy`, `_execute_tool` invokes a new `_auto_reindex_after_write` hook that re-parses the affected file(s) into the local SQLite index — no more "agent wrote a class but `code_search` can't find it" until the user manually runs `/index`. New `MetadataIndex.delete_outgoing_relationships(component_ids)` + `reindex_from_parse_result(result)` API: clears stale outgoing edges before re-upserting, so updated files (e.g. a class that adds a method calling a new dependency) replace their REFERENCES set rather than accumulating stale edges. New `embed_components(component_ids=...)` filter — auto-embed runs only over the rows we just touched, not the full table. Auto-embed gating: skips on `mock_org=True`, skips when `GOOGLE_API_KEY` is unset (graceful degrade — index updated, embeddings deferred until next `/index`), best-effort wrapped in try/except so a single Gemini hiccup never breaks the tool flow. `sf_source_deploy` walks the deploy `source_path` recursively, indexing every file with a registered parser (covers manual-edit-then-deploy). Failed deploys (`result.error` set) skip reindex. New `render_reindex_summary` in `repl_ui.py` prints a one-liner like `│ ↻ indexed 1 component, 1 relationship, 1 embedded` between the diff render and the OK line. 17 new tests in `test_auto_reindex.py` covering the full surface — happy paths, hash-gate verification across writes, parser exceptions, index-open failures, end-to-end through `_execute_tool`, mock-mode skip, missing-API-key skip, deploy success vs failure, and the new `component_ids` embed filter. Known limitations (out of scope): rename/move (old component id stays until full delta refresh); `file_delete` doesn't yet remove the row. |
| Minimal-repo distribution variant | Nice-to-have | 1-2 days | A stripped-down fetchable repo containing only what's needed to run sf-dev-agent — no `docs/`, no `sessions/`, no ROADMAP / PRESSURE_TEST / ARCHITECTURE / PROJECT_SUMMARY. Keeps `src/`, `tests/`, `pyproject.toml`, `uv.lock`, `.env.example`, `LICENSE`, plus a thin `README.md` that points back at the canonical repo for full context. Open design questions: (a) `scripts/build_minimal_repo.py` that produces a clean export folder vs. a sibling published repo with sync automation; (b) whether to ship a flat checkpoint or maintain it on every release; (c) `.gitattributes export-ignore` is a thin partial answer — only helps `git archive`, not `git clone`. Recommend: build-script approach in v1 (run on demand, output to `dist/sf-dev-agent-min/`), publish as a sibling GitHub repo only if there's actual demand. |
