...
def cmd_verbose(session: ReplSession, argv: list[str]) -> ReplDirective:
    ...
    REPL default is WARNING (set by __main__.py) so agent-iteration +
    httpx INFO chatter doesn't mingle with tool rendering. `on` raises
    to DEBUG; `off` returns to WARNING.
    """
    ...
    # httpx / httpcore are pinned to WARNING by default; restore that
    # gate when leaving DEBUG so the cycle is clean.
    if target == logging.DEBUG:
